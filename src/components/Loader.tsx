...
import type { TelegramState } from "../store/telegramSlice"

const Loader: React.FC = () => {
  const { t } = useTranslation()
  const theme = useSelector((state: RootState) => (state as { telegram: TelegramState }).telegram.theme)
  ...
  // Use the theme from Redux without a default value to ensure consistency
    ...
    <div className={`fixed inset-0 flex flex-col items-center justify-center ${overlayBg} z-50 w-full`}>
      <div className={`flex flex-col items-center justify-center w-full min-h-screen ${bgColor}`}>
        {showText ? (
          ...
          <img src={currentLogo || "/placeholder.svg"} alt="Turnify Logo" className="h-24 w-auto object-cover mb-4" />
        )}
      </div>
      <div className={`fixed bottom-8 px-4 ${footerTextColor} text-sm w-full text-center`}>
        ...
        <div className="flex mt-1 justify-center text-xs max-w-[400px] mx-auto">
          <Link to={"/terms"} className="text-blue-500 pr-3">
            {t("terms")}
          </Link>{" "}
          |{" "}
          <Link to={"/privacy"} className="text-blue-500 pl-3">
            {t("privacy")}
          </Link>
        </div>
      </div>
